...
#
# SYNC MODE (new):
#   - Always upsert what is in --ids-file
#   - Optionally delete anything in DB that is NOT in --ids-file (use --prune-not-in-file)
#
# Examples:
#   python seed_titles.py --mode movies --ids-file movie_tmdb_ids.txt --skip-existing-tmdb --verbose
#   python seed_titles.py --mode movies --ids-file movie_tmdb_ids.txt --prune-not-in-file --verbose
#
#   python seed_titles.py --mode tv --ids-file tv_tmdb_ids.txt --skip-existing-tmdb --verbose
#   python seed_titles.py --mode tv --ids-file tv_tmdb_ids.txt --prune-not-in-file --verbose
#   python seed_titles.py --mode tv --ids-file tv_tmdb_ids.txt --with-episode-imdb   (SLOW)
...
TMDB_KEY = "f6988ac086c88bbfe779ab0ed2eed215"  # your key
...
def tv_title_links(tmdb_id: int) -> Dict[str, str]:
    """ Title-level default links (S1E1 with selector=true), all using TMDB id. """
    return {
...
def normalize_to_tmdb_set(mode: str, raw_ids: List[str]) -> Set[int]:
    """
    You said your list is TMDB ids.
    But this keeps backward compat: if a line starts with tt..., we resolve to TMDB.
    """
    out: Set[int] = set()
    for raw in raw_ids:
        raw = raw.strip()
        ...
        if raw.startswith("tt"):
            mid, tid = tmdb_find_by_imdb(raw)
            tmdb_id = mid if mode == "movies" else tid
            if tmdb_id:
                out.add(int(tmdb_id))
            continue
...
# ========= PRUNE (delete titles not in list) =========

def prune_db_not_in_list(mode: str, keep_tmdb_ids: Set[int], verbose: bool = False) -> int:
    """
    Delete Titles of that type whose tmdb_id is not in keep_tmdb_ids.
    For TV, we also delete related seasons/episodes/extras first (safe even if CASCADE exists).
    Returns number of deleted Title rows.
    """
    ttype = "movie" if mode == "movies" else "tv"
    qs = Title.objects.filter(type=ttype).exclude(tmdb_id__in=list(keep_tmdb_ids))

    to_delete = list(qs.values_list("id", "tmdb_id"))
    if not to_delete:
        return 0

    deleted_count = 0
    for title_id, tmdb_id in to_delete:
        if verbose:
            print(f"[PRUNE] deleting {ttype} tmdb={tmdb_id} (title_id={title_id})")

        if ttype == "tv":
            # delete episodes -> seasons -> extras -> title
            Season_qs = Season.objects.filter(tv_id=title_id)
            Episode.objects.filter(season__in=Season_qs).delete()
            Season_qs.delete()
            TVShowExtras.objects.filter(title_id=title_id).delete()

        Title.objects.filter(id=title_id).delete()
        deleted_count += 1
...
def main():
    ap = argparse.ArgumentParser(description="Seed Titles: movies (TMDB/IMDb) and tv (TMDB/IMDb) + optional prune.")
    ...
    ap.add_argument(
        "--prune-not-in-file",
        action="store_true",
        help="DELETE from DB any title of this mode whose tmdb_id is NOT in --ids-file (DANGEROUS)"
    )

    args = ap.parse_args()

    raw_ids = load_ids_from_file(args.ids_file)
    desired_tmdb_ids = normalize_to_tmdb_set(args.mode, raw_ids)

    if args.verbose:
        print(f"[INPUT] mode={args.mode} lines={len(raw_ids)} desired_unique_tmdb={len(desired_tmdb_ids)}")

    created = updated = skipped = 0

    if args.mode == "movies":
        for tmdb_id in sorted(desired_tmdb_ids):
            try:
                ...
                print(f"[WARN] tmdb={tmdb_id} -> {e}")
                skipped += 1

        print(f"\n[DONE movies] created={created} updated={updated} skipped={skipped}")

    if args.mode == "tv":
        for tv_tmdb_id in sorted(desired_tmdb_ids):
            try:
                if args.skip_existing_tmdb and tv_exists_tmdb(tv_tmdb_id):
                    if args.verbose:
                        print(f"[SKIP] tv tmdb={tv_tmdb_id} already in DB")
                    skipped += 1
                    continue

                if args.verbose:
                    print(f"[FETCH] tv tmdb={tv_tmdb_id} ...")

                tv = tmdb_tv_full(tv_tmdb_id)
                res = upsert_tv_from_tmdb(
                    tv,
                    overwrite=args.overwrite,
                    with_episode_imdb=args.with_episode_imdb,
                ...
                print(f"[{res}] {tv.get('name')} (first_air_date={tv.get('first_air_date')}) tmdb={tv_tmdb_id}")

            except Exception as e:
                print(f"[WARN] tmdb={tv_tmdb_id} -> {e}")
                skipped += 1

        print(f"\n[DONE tv] created={created} updated={updated} skipped={skipped}")

    # PRUNE
    if args.prune_not_in_file:
        deleted = prune_db_not_in_list(args.mode, desired_tmdb_ids, verbose=args.verbose)
        print(f"\n[PRUNE DONE] deleted={deleted} (mode={args.mode})")
